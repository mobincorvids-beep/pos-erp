...
export function RealEstatePage() {
  ...
  return (
    <div>
      <p className="page-title mb-1">{t('realEstate.title')}</p>
      <p className="text-sm text-ink-muted mb-5">{t('realEstate.subtitle')}</p>
      <div className="flex gap-1 border-b border-rule mb-5">
        {[['leases', t('realEstate.leases')], ['properties', t('realEstate.properties')]].map(([key, label]) => (
      ...
      <div className="flex-1 min-w-0">
        <div className="flex justify-end mb-3">
          <button className="btn-primary" onClick={() => setShowForm(true)}>
            <span className="material-symbols-outlined text-base leading-none">add</span> {t('realEstate.startALease')}
          </button>
        </div>
        {loading && <Loading />}
        {!loading && leases.length === 0 && <EmptyState title={t('realEstate.noLeasesYet')} action={<button className="btn-primary" onClick={() => setShowForm(true)}>{t('realEstate.startOne')}</button>} />}
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-rule text-left text-xs text-ink-muted uppercase tracking-wide bg-surface-sunken">
                  <th className="px-3 py-2.5 font-semibold">{t('realEstate.property')}</th>
                  <th className="px-3 py-2.5 font-semibold">{t('realEstate.tenant')}</th>
                  <th className="px-3 py-2.5 font-semibold text-right">{t('realEstate.rent')}</th>
                  <th className="px-3 py-2.5 font-semibold">{t('realEstate.status')}</th>
                </tr>
              </thead>
              <tbody>
                {leases.map((l) => (
    ...
    try {
      ...
      if (form.securityDeposit) {
        ...
        if (!form.depositReceivedAccountId || !form.securityDepositLiabilityAccountId) {
          throw new Error(t('realEstate.depositAccountsRequired'));
        }
      } else {
      ...
      toast(t('realEstate.leaseStarted'), 'success');
      onSaved();
    } catch (err) {
  ...
  return (
    <div className="fixed inset-0 bg-ink/20 flex items-center justify-center z-40 px-4">
      <form onSubmit={handleSubmit} className="card p-5 w-full max-w-sm max-h-[85vh] overflow-y-auto">
        <p className="font-display text-lg font-bold text-ink mb-4">{t('realEstate.startALease')}</p>
        <div className="space-y-3">
          <select required className="field-input" value={form.branchId} onChange={(e) => setForm({ ...form, branchId: e.target.value })}>
            <option value="">{t('realEstate.branchPlaceholder')}</option>
            {branches.map((b) => <option key={b._id} value={b._id}>{b.name}</option>)}
          </select>
          <select required className="field-input" value={form.propertyId} onChange={(e) => setForm({ ...form, propertyId: e.target.value })}>
            <option value="">{t('realEstate.availablePropertyPlaceholder')}</option>
          ...
          <select required className="field-input" value={form.tenantCustomerId} onChange={(e) => setForm({ ...form, tenantCustomerId: e.target.value })}>
            <option value="">{t('realEstate.tenantPlaceholder')}</option>
          ...
          <div className="grid grid-cols-2 gap-2">
            <input type="number" required className="field-input num" placeholder={t('realEstate.monthlyRent')} value={form.monthlyRent} onChange={(e) => setForm({ ...form, monthlyRent: e.target.value })} />
            <input type="number" className="field-input num" placeholder={t('realEstate.lateFeePerDay')} value={form.lateFeePerDay} onChange={(e) => setForm({ ...form, lateFeePerDay: e.target.value })} />
          </div>

          <div className="tear-line" />
          <p className="text-xs text-ink-muted">{t('realEstate.securityDepositHint')}</p>
          <input type="number" className="field-input num" placeholder={t('realEstate.securityDepositPlaceholder')} value={form.securityDeposit} onChange={(e) => setForm({ ...form, securityDeposit: e.target.value })} />
          {form.securityDeposit && (
            <div className="grid grid-cols-2 gap-2">
              <select className="field-input" value={form.depositReceivedAccountId} onChange={(e) => setForm({ ...form, depositReceivedAccountId: e.target.value })}>
                <option value="">{t('realEstate.receivedIntoPlaceholder')}</option>
              ...
              <select className="field-input" value={form.securityDepositLiabilityAccountId} onChange={(e) => setForm({ ...form, securityDepositLiabilityAccountId: e.target.value })}>
                <option value="">{t('realEstate.liabilityAccountPlaceholder')}</option>
        ...
        <div className="flex justify-end gap-2 mt-5">
          <button type="button" className="btn-secondary" onClick={onClose}>{t('realEstate.cancel')}</button>
          <button type="submit" disabled={saving} className="btn-primary">{saving ? t('realEstate.starting') : t('realEstate.startLease')}</button>
        </div>
      </form>
    </div>
  );
...
function LeasePanel({ lease, onClose, onChanged }) {
  ...
  async function generateRent() {
    ...
    try {
      ...
      if (!product) { toast(t('realEstate.selectBillingProduct'), 'error'); setBusy(false); return; }
      ...
      const billedMsg = t('realEstate.billed', { amount: formatMoney(result.sale.totalAmount, company?.currency) });
      toast(`${billedMsg}${result.lateFee > 0 ? ` ${t('realEstate.includesLateFee', { amount: formatMoney(result.lateFee, company?.currency) })}` : ''}`, 'success');
      onChanged();
  ...
  async function endLease(e) {
    ...
    try {
      ...
      toast(result.refund > 0 ? t('realEstate.leaseEndedWithRefund', { amount: formatMoney(result.refund, company?.currency) }) : t('realEstate.leaseEnded'), 'success');
      onChanged(); onClose();
    } catch (err) { toast(err.message, 'error'); } finally { setBusy(false); }
  }
  ...
  return (
    <div className="w-full lg:w-96 shrink-0 card p-4 h-fit">
      <div className="flex items-center justify-between mb-3">
        ...
        <button className="text-ink-muted hover:text-ink text-sm" onClick={onClose}>{t('realEstate.close')}</button>
      </div>
      <p className="text-sm text-ink-muted mb-4">{lease.tenantCustomerId?.name}: <span className="num">{formatMoney(lease.monthlyRent, company?.currency)}</span>{t('realEstate.perMonth')}</p>
      {lease.securityDeposit > 0 && (
        <div className="chip-info mb-4 !inline-flex">{t('realEstate.securityDepositHeld')}: <span className="num ml-1">{formatMoney(lease.securityDeposit, company?.currency)}</span></div>
      )}

      {lease.status === 'active' && !showEndForm && (
        <>
          <p className="eyebrow mb-2">{t('realEstate.generateRentInvoice')}</p>
          <div className="space-y-2 mb-2">
            <select className="field-input" value={billingProductId} onChange={(e) => setBillingProductId(e.target.value)}>
              <option value="">{t('realEstate.billingProductPlaceholder')}</option>
              {products.map((p) => <option key={p._id} value={p._id}>{p.name}</option>)}
            </select>
            <select className="field-input" value={warehouseId} onChange={(e) => setWarehouseId(e.target.value)}>
              <option value="">{t('realEstate.warehousePlaceholder')}</option>
              {warehouses.map((w) => <option key={w._id} value={w._id}>{w.name}</option>)}
            </select>
          </div>
          <button className="btn-primary w-full mb-3" disabled={!billingProductId || !warehouseId || busy} onClick={generateRent}>
            {busy ? t('realEstate.generating') : t('realEstate.generateRent')}
          </button>
          <button className="btn-secondary w-full" disabled={busy} onClick={() => lease.securityDeposit > 0 ? setShowEndForm(true) : endLease()}>{t('realEstate.endLease')}</button>
        </>
      )}

      {lease.status === 'active' && showEndForm && (
        <form onSubmit={endLease} className="space-y-2">
          <p className="text-sm text-ink-muted">{t('realEstate.depositOnFileHint')}</p>
          <div>
            <label className="field-label">{t('realEstate.deductionForDamage')}</label>
          ...
          <div>
            <label className="field-label">{t('realEstate.refundFromAccount')}</label>
            <select required className="field-input" value={refundAccountId} onChange={(e) => setRefundAccountId(e.target.value)}>
              <option value="">{t('realEstate.select')}</option>
              {accounts.filter((a) => a.isPaymentAccount).map((a) => <option key={a._id} value={a._id}>{a.name}</option>)}
            </select>
          </div>
          {Number(deductionAmount) > 0 && (
            <div>
              <label className="field-label">{t('realEstate.forfeitedRevenueAccount')}</label>
          ...
          <div className="flex gap-2 pt-1">
            <button type="button" className="btn-secondary flex-1" onClick={() => setShowEndForm(false)}>{t('realEstate.back')}</button>
            <button type="submit" disabled={busy} className="btn-danger flex-1">{busy ? t('realEstate.ending') : t('realEstate.confirmEnd')}</button>
          </div>
        </form>
      )}

      {lease.status === 'ended' && <p className="text-sm text-ink-muted">{t('realEstate.leaseEnded')}</p>}
    </div>
  );
}
...
function PropertiesTab() {
  ...
  return (
    <div>
      <div className="flex justify-end mb-3">
        <button className="btn-primary" onClick={() => setShowForm(true)}>
          <span className="material-symbols-outlined text-base leading-none">add</span> {t('realEstate.addProperty')}
        </button>
      </div>
      {loading && <Loading />}
      {!loading && properties.length === 0 && <EmptyState title={t('realEstate.noPropertiesYet')} action={<button className="btn-primary" onClick={() => setShowForm(true)}>{t('realEstate.addOne')}</button>} />}
...
function PropertyForm({ onClose, onSaved }) {
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      toast(t('realEstate.propertyAdded'), 'success');
      onSaved();
    } catch (err) {
      toast(err.message, 'error');
    } finally {
  ...
  return (
    <div className="fixed inset-0 bg-ink/20 flex items-center justify-center z-40 px-4">
      <form onSubmit={handleSubmit} className="card p-5 w-full max-w-sm">
        <p className="font-display text-lg font-bold text-ink mb-4">{t('realEstate.addProperty')}</p>
        <div className="space-y-3">
          <select required className="field-input" value={form.branchId} onChange={(e) => setForm({ ...form, branchId: e.target.value })}>
            <option value="">{t('realEstate.branchPlaceholder')}</option>
            {branches.map((b) => <option key={b._id} value={b._id}>{b.name}</option>)}
          </select>
          <input required className="field-input" placeholder={t('realEstate.unitNumber')} value={form.unitNumber} onChange={(e) => setForm({ ...form, unitNumber: e.target.value })} />
          <input required className="field-input" placeholder={t('realEstate.propertyType')} value={form.propertyType} onChange={(e) => setForm({ ...form, propertyType: e.target.value })} />
        </div>
        <div className="flex justify-end gap-2 mt-5">
          <button type="button" className="btn-secondary" onClick={onClose}>{t('realEstate.cancel')}</button>
          <button type="submit" disabled={saving} className="btn-primary">{saving ? t('realEstate.saving') : t('realEstate.save')}</button>
        </div>
      </form>
    </div>
  );
