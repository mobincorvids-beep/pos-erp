  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-4">
        ...
        <button className="btn-primary" onClick={() => setShowForm(true)}>New cost center</button>
      </div>
      <p className="text-sm text-ink-muted mb-5 max-w-2xl">Tag any voucher entry with a cost center — a department, a project, an arbitrary internal grouping — to see a real profit &amp; loss scoped to just that entry, even when it shares a voucher with untagged or differently-tagged money.</p>
            ...
            <tbody>
              {costCenters.map((c) => (
                <tr key={c._id} className="border-b border-rule last:border-0">
                  ...
                  <td className="px-3 py-2 text-ink-muted">{c.code || '—'}</td>
                  <td className="px-3 py-2 text-right">
                    <button className="btn-ghost !text-accent" onClick={() => setViewing(c)}>View P&amp;L</button>
                  </td>
                </tr>
              ))}
            </tbody>
  ...
  return (
    <div className="fixed inset-0 bg-ink/20 flex items-center justify-center z-40 px-4">
      <form onSubmit={handleSubmit} className="card p-5 w-full max-w-sm">
        <p className="font-display text-lg mb-4">New cost center</p>
  ...
  return (
    <div className="fixed inset-0 bg-ink/20 flex items-center justify-center z-40 px-4">
      <div className="card p-5 w-full max-w-lg max-h-[85vh] overflow-y-auto">
        <div className="flex items-center justify-between mb-4">
          <p className="font-display text-lg">{costCenter.name} — P&amp;L</p>
          ...
          <>
            <div className="grid grid-cols-3 gap-3 mb-4">
              <div className="card p-3">
                <p className="text-xs text-ink-muted uppercase tracking-wide">Income</p>
                <p className="font-display text-lg mt-1 num">{formatMoney(report.totalIncome, company?.currency)}</p>
              </div>
              <div className="card p-3">
                <p className="text-xs text-ink-muted uppercase tracking-wide">Expenses</p>
                <p className="font-display text-lg mt-1 num">{formatMoney(report.totalExpenses, company?.currency)}</p>
              </div>
              <div className="card p-3">
                <p className="text-xs text-ink-muted uppercase tracking-wide">Net</p>
                <p className={`font-display text-lg mt-1 num ${report.netProfit < 0 ? 'text-danger' : 'text-accent-strong'}`}>{formatMoney(report.netProfit, company?.currency)}</p>
              </div>
            </div>
