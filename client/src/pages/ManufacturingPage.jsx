...
export function ManufacturingPage() {
  ...
  return (
    <div>
      <p className="page-title mb-4">Manufacturing</p>
  ...
  return (
    <div className="flex flex-col lg:flex-row gap-6">
      <div className="flex-1 min-w-0">
        ...
        {loading && <Loading />}
        {!loading && orders.length === 0 && <EmptyState title="No work orders yet" description="Create a Bill of Materials first, then start a production run against it." />}
        {!loading && orders.length > 0 && (
          <div className="card overflow-hidden">
            <table className="w-full text-sm">
              ...
              <tbody>
                {orders.map((wo) => (
                  <tr key={wo._id} onClick={() => setSelected(wo)} className={`border-b border-rule last:border-0 cursor-pointer hover:bg-paper ${selected?._id === wo._id ? 'bg-accent-soft/40' : ''}`}>
                    <td className="px-3 py-2 num">{wo.workOrderNumber}</td>
                    <td className="px-3 py-2"><span className="chip-neutral">{wo.status.replace('_', ' ')}</span></td>
                    <td className="px-3 py-2 num text-right">{formatQty(wo.quantityToProduce)}</td>
                    <td className="px-3 py-2 num text-right">{formatQty(wo.quantityProduced)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
  ...
  return (
    <div className="w-full lg:w-80 shrink-0 card p-4 h-fit">
      <div className="flex items-center justify-between mb-3">
        <p className="font-display text-lg num">{workOrder.workOrderNumber}</p>
        ...
        <div className="grid grid-cols-2 gap-3">
          {boms.map((b) => (
            <div key={b._id} className="card p-3">
              ...
              <p className="text-xs text-ink-muted mt-1">{b.components.length} component{b.components.length === 1 ? '' : 's'}</p>
