...
export function StockCountsPage() {
  ...
  async function deleteCount(c) {
    if (!window.confirm(`Delete stocktake ${c.countNumber}? This cannot be undone.`)) return;
    try {
                      ...
                      >
                        ...
                        <td className="py-3 px-4 text-right">
                          {c.status === 'in_progress' ? (
                            ...
                            <span className="text-ink-muted/40" title="Submitted stocktakes have posted stock adjustments and can't be deleted.">
                              <span className="font-icon text-[18px] leading-none">delete</span>
                            </span>
                          )}
                        </td>
...
function StockCountPanel({ count, onClose, onChanged }) {
  ...
  async function removeItem(item) {
    if (!window.confirm(`Remove "${item.productId?.name || 'this item'}" from the stocktake?`)) return;
    setBusy(true);
    try {
  ...
  async function deleteCount() {
    if (!window.confirm(`Delete stocktake ${count.countNumber}? This cannot be undone.`)) return;
    setBusy(true);
    try {
          ...
          <div key={item._id} className="flex items-center justify-between text-sm gap-2 px-2 py-2 rounded-lg hover:bg-surface-sunken">
            ...
            {count.status === 'in_progress' && (
              <button
                ...
                title="Remove item from stocktake"
                disabled={busy}
