  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-4">
        ...
        <button className="btn-primary" onClick={() => setShowForm(true)}>New template</button>
      </div>
            ...
            <tbody>
              {templates.map((t) => (
                <tr key={t._id} className="border-b border-rule last:border-0">
                  <td className="px-3 py-2">{t.customerId?.name || '—'}</td>
                  <td className="px-3 py-2 text-ink-muted">{FREQUENCY_LABEL[t.frequency]}</td>
                  <td className="px-3 py-2 text-ink-muted">{t.items.length} item{t.items.length !== 1 ? 's' : ''}</td>
                  <td className="px-3 py-2 text-ink-muted">{formatDate(t.nextRunDate)}</td>
                  <td className="px-3 py-2"><span className={STATUS_CHIP[t.status]}>{t.status}</span></td>
                  <td className="px-3 py-2 text-right">
                    {t.status === 'active' && <button className="btn-ghost !text-warning" onClick={() => changeStatus(t._id, 'pause')}>Pause</button>}
                    {t.status === 'paused' && <button className="btn-ghost !text-accent" onClick={() => changeStatus(t._id, 'resume')}>Resume</button>}
                    {t.status !== 'cancelled' && <button className="btn-ghost !text-danger" onClick={() => changeStatus(t._id, 'cancel')}>Cancel</button>}
                  </td>
                </tr>
              ))}
            </tbody>
  ...
  return (
    <div className="fixed inset-0 bg-ink/20 flex items-center justify-center z-40 px-4">
      <form onSubmit={handleSubmit} className="card p-5 w-full max-w-lg max-h-[85vh] overflow-y-auto">
        <p className="font-display text-lg mb-4">New recurring invoice</p>
