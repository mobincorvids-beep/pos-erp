...
function CustomerForm({ customer, onClose, onSaved }) {
  ...
  function addTag() {
    ...
    if (!value) return;
    if (form.tags.includes(value)) { setTagDraft(''); return; }
    if (form.tags.length >= 10) return;
    setForm((f) => ({ ...f, tags: [...f.tags, value.slice(0, 30)] }));
    setTagDraft('');
  }

  function removeTag(tag) {
    setForm((f) => ({ ...f, tags: f.tags.filter((tg) => tg !== tag) }));
  }
          ...
          <div>
            ...
            {form.tags.length > 0 && (
              <div className="flex flex-wrap gap-1 mb-2">
                {form.tags.map((tag) => (
                  <span key={tag} className="chip-neutral inline-flex items-center gap-1">
                    {tag}
                    <button
                      type="button"
                      className="text-ink-muted hover:text-danger leading-none"
                      onClick={() => removeTag(tag)}
                      aria-label={t('customers.removeTag', { tag })}
                    >
                      ×
                    </button>
                  </span>
            ...
            <div className="flex gap-2">
              ...
              />
              <button type="button" className="btn-secondary" onClick={addTag} disabled={!tagDraft.trim() || form.tags.length >= 10}>
                {t('customers.addTag')}
              </button>
            </div>
          </div>
