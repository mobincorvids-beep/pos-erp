...
export function JewelryPage() {
  ...
  return (
    <div>
      <p className="page-title mb-4">Jewelry</p>
      <div className="flex gap-1 border-b border-rule mb-5">
        {[['rates', 'Gold rates'], ['items', 'Item pricing'], ['buybacks', 'Buy-backs']].map(([key, label]) => (
...
function RatesTab() {
  ...
  return (
    <div className="grid grid-cols-2 gap-6">
      <form onSubmit={setRate} className="card p-4">
        <p className="text-sm font-medium mb-3">Set today's rate</p>
        ...
        <div className="space-y-1">
          {rates.map((r) => (
            <div key={r._id} className="flex justify-between text-sm">
              <span>{r.karat}k</span>
              <span className="num">{formatMoney(r.ratePerGram, company?.currency)}/g</span>
            </div>
          ))}
        </div>
      </div>
...
function ItemsTab() {
  ...
  return (
    <div className="grid grid-cols-2 gap-6">
      <form onSubmit={configure} className="card p-4">
        <p className="text-sm font-medium mb-3">Configure item pricing</p>
      ...
      <div className="card p-4">
        <p className="text-sm font-medium mb-3">Live quote</p>
        ...
        {quote && (
          <div className="space-y-1.5 text-sm">
            ...
            <div className="flex justify-between text-base font-medium"><span>Total</span><span className="num text-accent-strong">{formatMoney(quote.totalPrice, company?.currency)}</span></div>
  ...
  return (
    <div className="grid grid-cols-2 gap-6">
      <div className="card p-4">
        ...
        <select className="field-input mb-4" value={selectedCustomer} onChange={(e) => setSelectedCustomer(e.target.value)}>
          <option value="">Select…</option>
          {customers.map((c) => <option key={c._id} value={c._id}>{c.name}</option>)}
        </select>

        <form onSubmit={intake}>
          <p className="text-sm font-medium mb-2">Intake old gold</p>
      ...
      <div className="card p-4">
        <p className="text-sm font-medium mb-3">This customer's buy-backs</p>
        ...
        <div className="space-y-2">
          {buybacks.map((b) => (
            <div key={b._id} className="flex justify-between text-sm border-b border-rule pb-1.5">
              <span className="text-ink-muted">{b.weightGrams}g @ {b.karat}k</span>
              <span className="num">{formatMoney(b.creditAmount, company?.currency)} <span className={b.status === 'applied' ? 'chip-accent' : 'chip-neutral'}>{b.status}</span></span>
            </div>
          ))}
        </div>
