  ...
  return (
    <div>
      <div className="flex items-end justify-between mb-5 gap-3 flex-wrap">
        <div>
          <p className="eyebrow mb-1">Hardware</p>
          <p className="page-title">Tool rentals</p>
        </div>
        <button className="btn-primary" onClick={() => setShowForm(true)}>Check out rental</button>
      </div>
                    ...
                    <td className="px-4 py-3 text-right">
                      {r.status === 'out' && (
                        <div className="inline-flex gap-1">
                          <button className="btn-ghost !text-accent" onClick={() => setReturning(r)}>Return</button>
                          <button className="btn-ghost !text-danger" onClick={() => handleVoid(r)}>Void</button>
                        </div>
                      )}
                    </td>
                  </tr>
  ...
  return (
    <div className="fixed inset-0 bg-ink/20 flex items-center justify-center z-40 px-4">
      <form onSubmit={handleSubmit} className="card p-5 w-full max-w-sm max-h-[85vh] overflow-y-auto">
        <p className="font-display text-lg text-ink mb-4">Check out rental</p>
        <div className="space-y-3">
          ...
          <div>
            <label className="field-label">Item to rent</label>
          ...
          <div className="grid grid-cols-2 gap-2">
            <div><label className="field-label">Daily rate</label><input type="number" required placeholder="Daily rate" className="field-input num" value={form.dailyRate} onChange={(e) => setForm({ ...form, dailyRate: e.target.value })} /></div>
            <div><label className="field-label">Deposit</label><input type="number" required placeholder="Deposit" className="field-input num" value={form.depositAmount} onChange={(e) => setForm({ ...form, depositAmount: e.target.value })} /></div>
          </div>
          <div>
          ...
          <div>
            <label className="field-label">Deposit received into</label>
          ...
          <div>
            <label className="field-label">Usage billing product</label>
  ...
  return (
    <div className="fixed inset-0 bg-ink/20 flex items-center justify-center z-40 px-4">
      <form onSubmit={handleSubmit} className="card p-5 w-full max-w-sm">
        <p className="font-display text-lg text-ink mb-4">Return rental</p>
        <div className="space-y-3">
          <div>
          ...
          {condition === 'minor_damage' && (
            <div><label className="field-label">Forfeit %</label><input type="number" placeholder="Forfeit %" className="field-input num" value={forfeitPercentForMinorDamage} onChange={(e) => setForfeitPercentForMinorDamage(e.target.value)} /></div>
          )}
          {condition !== 'good' && (
            <div><label className="field-label">Damage revenue account</label><select required className="field-input" value={damageRevenueAccountId} onChange={(e) => setDamageRevenueAccountId(e.target.value)}><option value="">Damage revenue account…</option>{accounts.map((a) => <option key={a._id} value={a._id}>{a.name}</option>)}</select></div>
          )}
          <div>
            <label className="field-label">Final payment account</label>
