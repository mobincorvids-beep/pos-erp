...
app.use(express.json({ limit: '16mb' })); // explicit rather than relying on express's default — raised from 8mb to comfortably fit a single inline document upload (fileData is capped at 10MB pre-base64, ~13.7MB once base64-encoded, plus JSON overhead), while still blunting a body-based DoS attempt
